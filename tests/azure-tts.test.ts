...
test('azure: SSML 逸出與詞典包裝，長詞優先', () => {
  const lex = { version: 1, locale: 'zh-TW', entries: { '垃圾': 'ㄌㄜˋ ㄙㄜˋ', '垃圾桶': 'ㄌㄜˋ ㄙㄜˋ ㄊㄨㄥˇ' } };
  const out = applyLexicon('把垃圾丟進垃圾桶 <b>', lex);
  assert.equal(out, '把<phoneme alphabet="sapi" ph="ㄌㄜˋ ㄙㄜˋ">垃圾</phoneme>丟進<phoneme alphabet="sapi" ph="ㄌㄜˋ ㄙㄜˋ ㄊㄨㄥˇ">垃圾桶</phoneme> &lt;b&gt;');
  ...
  assert.match(buildSsml('a', { voice: 'v', locale: 'zh-TW', rate: 1 }), /<voice name="v">a<\/voice>/); // 原速不包 prosody
});
...
test('azure: 沒金鑰就明確失敗；HTTP 錯誤分流；成功回傳時長', async () => {
  ...
  const ok = new AzureTtsProvider({
    ...
    lexicon: { version: 1, locale: 'zh-TW', entries: { '垃圾': 'ㄌㄜˋ ㄙㄜˋ' } },
